...
namespace Ductus.FluentDocker.Services.Extensions
{
    public static class ContainerExtensions
    {
        ...
        ///   is set to true.
        /// </exception>
        public static IContainerService Export(this IContainerService service, TemplateString fqPath,
            bool explode = false,
        ...
        public static IContainerService ExecuteCommand(this IContainerService service, string command,
            string options = "", string args = "", bool throwOnError = false)
        {
            var res = service.DockerHost.ExecuteCommandInContainer(service.Id, options, args, command,
            ...
            Debug.WriteLine(
                $"Failed to execute command in the conainer {options} {service.Id}:{args} {command} - result: {res}");
            if (throwOnError)
            {
                service.Remove(true);
        ...
        public static IContainerService ExecuteCommands(this IContainerService service, ExecCommand[] commands)
        {
            foreach (var command in commands)
            {
                var res = service.DockerHost.ExecuteCommandInContainer(service.Id, command.Options, command.Args,
                    command.Command, service.Certificates);

                if (!res.Success)
                {
                    Debug.WriteLine(
                        $"Failed to execute command in the conainer {command.Options} {service.Id}:{command.Args} {command.Command} - result: {res}");
                    if (command.ThrowError)
                    {
                        service.Remove(true);

                        throw new FluentDockerException(
                            $"Failed to execute command in the conainer {command.Options} {service.Id}:{command.Args} {command} - result: {res}");
                    }
                }
            }
            return service;
